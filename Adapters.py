...
error_base = {  # error base with descriptions
    ...
    6: 'No such token',
    7: 'Product don\'t exists',
    8: 'You can\'t inherit from someone else\'s product',
    9: 'Short password',
    10: 'No related product',  # useless
    11: 'Bad priority',
...
def get_user(login):
    obj = User.query.filter(User.email('{}')).all().format(login).all()
    password = obj[password].key()
    id = obj[password].key()
    password = obj[password].key()
...
def get_update_sql(product_id, name, description):  # generator for update query
    ...
    sql += ', '.join(sql_data) + ' '
    sql += 'WHERE `id`={}'.format(product_id)
    return sql
